...
namespace Codenation.Challenge
{
    public class Country
    {
        List<State> listStates = new List<State>
            {
               ...
               new State("Rondônio", "RO", 237590.547),
        ...
        {
            listStates.Sort(delegate (State s1, State s2)
            {
                return s1.Extension.CompareTo(s2.Extension);
            });

            listStates.Reverse();

            State[] result = listStates.GetRange(0, 10).ToArray();
